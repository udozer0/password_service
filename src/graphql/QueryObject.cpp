...
{
	return {
		{ R"gql(ping)gql"sv, [this](service::ResolverParams&& params) { return resolvePing(std::move(params)); } },
...
service::AwaitableResolver Query::resolvePing(service::ResolverParams&& params) const
{
	std::unique_lock resolverLock(_resolverMutex);
	service::SelectionSetParams selectionSetParams { static_cast<const service::SelectionSetParams&>(params) };
	auto directives = std::move(params.fieldDirectives);
	auto result = _pimpl->getPing(service::FieldParams { std::move(selectionSetParams), std::move(directives) });
	resolverLock.unlock();
...
{
	typeQuery->AddFields({
		schema::Field::Make(R"gql(ping)gql"sv, R"md()md"sv, std::nullopt, schema->WrapType(introspection::TypeKind::NON_NULL, schema->LookupType(R"gql(String)gql"sv)))
	});
}
